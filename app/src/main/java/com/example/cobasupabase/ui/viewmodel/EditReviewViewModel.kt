...
class EditReviewViewModel(
    savedStateHandle: SavedStateHandle
    // HAPUS repo dari sini agar constructor sesuai standar Android ViewModel
) : ViewModel() {

    // PINDAHKAN ke sini
    private val repo: ReviewRepository = ReviewRepository()

    // Ambil ID dengan aman (Safety Check)
    private val reviewId: Long = savedStateHandle.get<Long>("reviewId") ?: -1L
    ...
    private fun loadReviewData() {
        if (reviewId == -1L) {
            _loadState.value = UiResult.Error("Terjadi kesalahan navigasi: ID Review tidak ditemukan.")
            return
        }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (review != null) {
                    ...
                    _loadState.value = UiResult.Error("Data review tidak ditemukan.")
                }
            } catch (e: Exception) {
                _loadState.value = UiResult.Error(e.message ?: "Gagal memuat data review.")
            }
        }
    }
    ...
    fun updateReview() {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.value = UiResult.Error(e.message ?: "Gagal memperbarui review.")
            }
        }
    }
}